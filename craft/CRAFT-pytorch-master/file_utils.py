...
def saveResult(stringson, img_file, imag, boxes, dirname='./result/', verticals=None, texts=None):
        ...
        if doc_type == "СНИЛС":
            formal_words = ["российская", "федерация", "свидетельство", "обязательного", "пенсионного",
                            "осударственного", "страхования", "страховое", "ф.и.о.", "фио", "дата",
                            "место", "рождения", "пол", "регистрации", "ф.", "и.", "о.", "0."]
            all_words = []
            for i in one_line:
                for j in i:
                    flag = True
            ...
            while len(lst) != 0:
                for k in lst:
                    if len(k) == 1 or len(k) == 2:
                        ...
                        break
                    if k == "года":
                        stringson["client_mail_id"][doc_type]["Дата рождения"][-1] = ""
                        lst.remove(k)
                        break
                    if k.isnumeric() and len(k) == 4:
                        ...
                        stringson["client_mail_id"][doc_type]["Дата рождения"][1] = m_dict[k]
                        lst.remove(k)
                        break
            ...
            while fl:
                if all_words[cnt].lower() != "мужской":
                    if all_words[cnt].lower() != "женский":
            ...
            while len(lst) != 0:
                for k in lst:
                    if len(k) == 1 or len(k) == 2:
                        ...
                        break
                    if k == "года":
                        stringson["client_mail_id"][doc_type]["Дата регистрации"][-1] = ""
                        lst.remove(k)
                        break
                    if k.isnumeric() and len(k) == 4:
                        ...
                        stringson["client_mail_id"][doc_type]["Дата регистрации"][1] = m_dict[k]
                        lst.remove(k)
                        break
            stringson["client_mail_id"][doc_type]["Дата регистрации"] = " ".join(
                stringson["client_mail_id"][doc_type]["Дата регистрации"])

            stringson["client_mail_id"][doc_type]["Пол"] = all_words[-5+yflag]
